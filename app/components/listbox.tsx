...
export const dropdownItemStyles = tv({
  base: 'group flex items-center gap-3 cursor-default select-none p-3 md:gap-2 md:p-2 rounded-lg outline outline-0 text-sm forced-color-adjust-none',
  variants: {
    isDisabled: {
...
export function DropdownItem(props: ListBoxItemProps & { icon?: string }) {
  const textValue =
    props.textValue || (typeof props.children === 'string' ? props.children : undefined)
  const Icon = Iconoir[props.icon as keyof typeof Iconoir] as ComponentType<any>
  return (
    <AriaListBoxItem {...props} textValue={textValue} className={dropdownItemStyles}>
      {composeRenderProps(props.children, (children, { isSelected }) => (
        <>
          ...
          <span className='flex items-center'>{isSelected && <Check className='w-4 h-4' />}</span>
        </>
      ))}
    </AriaListBoxItem>
  )
